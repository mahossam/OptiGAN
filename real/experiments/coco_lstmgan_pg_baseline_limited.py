...
outdir = os.path.join(cwd, 'out', time.strftime("%Y%m%d"), dataset, f"lstmgan_rl_only_{rl_only}_pg_bline_{pg_baseline}_limited", datetime.utcnow().strftime("%H%M%S%f"))
...
call([executable, 'bleu_post_training.py', outdir, 'na'], env=my_env, cwd=rootdir)
