...
architecture = ['lstm_vanilla_new_init_contin', 'lstm_vanilla_new_init_contin', 'lstm_vanilla_new_init_contin', 'lstm_vanilla_new_init_contin', 'lstm_vanilla_new_init_contin', 'lstm_vanilla_new_init_contin', 'lstm_vanilla_new_init_contin', 'lstm_vanilla_new_init_contin']
...
gsteps = '1'
dsteps = '5'
gen_emb_dim = '16'  # Used as trajectory data input size in continuous case # '32'
dis_emb_dim = '64'
num_rep = '64'
sn = False
decay = False
...
outdir = os.path.join(cwd, 'out', time.strftime("%Y%m%d"), dataset, f"lstmgan_gan_only_{True}", datetime.utcnow().strftime("%H%M%S%f"))
...
args = [
    ...
    '--start-token', '0',
    '--seq-len', seq_len,
    '--num-sentences', '24',  # how many generated sentences to use per evaluation
...
# call([executable, 'bleu_post_training.py', os.path.join(outdir, 'samples'), 'na'], env=my_env, cwd=rootdir)
