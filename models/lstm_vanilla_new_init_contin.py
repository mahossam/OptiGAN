...
def generator(x_real, temperature, vocab_size, batch_size, seq_len, gen_emb_dim, mem_slots, head_size, num_heads,
    ...
    gen_x = tensor_array_ops.TensorArray(dtype=tf.float32, size=seq_len, dynamic_size=False, infer_shape=True)
    gen_x_sample = tensor_array_ops.TensorArray(dtype=tf.float32, size=seq_len, dynamic_size=False, infer_shape=True)
    ...
    def _gen_recurrence(i, x_t, h_tm1, gen_o, gen_x, gen_x_sample, gen_x_onehot_adv):
        ...
        # next_token = tf.stop_gradient(o_t)

        #Guassian sampling for inference
        next_token_sample = tf.stop_gradient(tf.random_normal((None, vocab_size), mean=o_t, stddev=0.1))

        x_onehot_appr = o_t  # one-hot-like, [batch_size x vocab_size]

        # x_tp1 = next_token  # outputs, [batch_size x output size (unconditional case)]
        x_tp1 = next_token_sample  # outputs, [batch_size x output size (unconditional case)]

        gen_o = gen_o.write(i, o_t)  # [batch_size], prob
        # gen_x = gen_x.write(i, next_token)  # indices, [batch_size]
        gen_x = gen_x.write(i, next_token_sample)  # indices, [batch_size]
        # gen_x_sample = gen_x_sample.write(i, next_token_sample)  # indices, [batch_size x vocab_size]
        gen_x_sample = gen_x_sample.write(i, 0.0)  # indices, [batch_size x vocab_size]
    ...
    _, _, _, gen_o, gen_x, gen_x_sample, gen_x_onehot_adv = control_flow_ops.while_loop(
        ...
        loop_vars=(tf.constant(0, dtype=tf.int32), start_tokens,
    ...
    gen_x_sample = tf.transpose(gen_x_sample.stack(), perm=[1, 0])  # batch_size x seq_len x vocab_size
    ...
    # x_emb = tf.transpose(x_real, perm=[1, 0, 2])  # seq_len x batch_size x emb_dim
    x_emb = tf.transpose(x_real, perm=[1, 0, 2])  # seq_len x batch_size x input_size
    ...
    g_predictions = tf.transpose(g_predictions.stack(), perm=[1, 0, 2])  # batch_size x seq_length x vocab_size (output_size)

    # pre-training loss
    rnn_loss = 0.0
    reinforce_loss = 0.0
    for time_step in range(seq_len):
        # rnn_loss_t = 0.5 * tf.reduce_sum(tf.square(rnn_outputs[:, time_step, :] - labels_tensor[:, time_step, :]), axis=1)
        # self.reinforce_loss += tf.reduce_mean(rnn_loss_t * self.rewards[:, time_step + 1])  # TODO[fixme] Was this a wrong implementation ? Revise the coninouts gaussian case with vanilla PG

        # if Config.args.L2_variant == 1:
        rnn_loss += tf.reduce_mean(tf.nn.l2_loss(g_predictions[:, time_step, :] - x_real[:, time_step, :]))
        # elif Config.args.L2_variant == 2:
        #     rnn_loss += tf.reduce_mean(rnn_loss_t)
        # elif Config.args.L2_variant == 3:
        #     rnn_loss += tf.reduce_sum(rnn_loss_t)

    g_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator')
    regul = 0.0
    for tf_var in g_vars:
        if not tf_var.name in ["bi", "bf", "bg", "bc", "bog"]:   # and "rnn" in tf_var.name:
            # print("added l2 reg for", tf_var.name)
            regul += tf.nn.l2_loss(tf_var)

    pretrain_loss = rnn_loss + 0.003 * regul
...
def create_recurrent_unit(emb_dim, hidden_dim, act="elu"):
    ...
    def unit(x, hidden_memory_tm1):
        ...
        if act=="elu":
            current_hidden_state = o * tf.nn.elu(c)
        else:
