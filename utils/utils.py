...
def save_samples(generated_samples, output_file=None):
    if output_file is not None:
        with open(output_file, 'w') as fout:
            for sent in generated_samples:
                buffer = ' '.join([str(x) for x in sent]) + '\n'
...
def samples_no_padding(generated_samples, eof_code):
    codes = list()
    for sent in generated_samples:
        # buffer = ' '.join([str(x) for x in sent]) + '\n'
        sent_new = list()
        for w in sent:
            if w != eof_code:
                sent_new.append(w)
        # fout.write(buffer)
        codes.append(sent_new)
    return codes


def init_sess(load_model=False, load_path="", saver=None):
...
def get_oracle_file(data_file, oracle_file, seq_len, word_index_dict):
...
def save_oracle_file_no_padding(data_file, oracle_file_no_padding, seq_len, word_index_dict):
    tokens = get_tokenlized(data_file)
    # word_set = get_word_list(tokens)
    # [word_index_dict, index_word_dict] = get_dict(word_set)
    with open(oracle_file_no_padding, 'w') as outfile:
        outfile.write(text_to_code_no_padding(tokens, word_index_dict, seq_len))
