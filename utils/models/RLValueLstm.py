...
class OracleLstm(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim, sequence_length, start_token):
        ...
        with tf.variable_scope('generator'):
            ...
            self.g_embeddings = tf.Variable(
                tf.random_normal([self.num_vocabulary, self.emb_dim], 0.0, 1.0))
    ...
    def create_recurrent_unit(self, params):
        ...
        self.Wi = tf.Variable(tf.random_normal([self.emb_dim, self.hidden_dim], 0.0, 1.0))
        self.Ui = tf.Variable(tf.random_normal([self.hidden_dim, self.hidden_dim], 0.0, 1.0))
        self.bi = tf.Variable(tf.random_normal([self.hidden_dim, ], 0.0, 1.0))

        self.Wf = tf.Variable(tf.random_normal([self.emb_dim, self.hidden_dim], 0.0, 1.0))
        self.Uf = tf.Variable(tf.random_normal([self.hidden_dim, self.hidden_dim], 0.0, 1.0))
        self.bf = tf.Variable(tf.random_normal([self.hidden_dim, ], 0.0, 1.0))

        self.Wog = tf.Variable(tf.random_normal([self.emb_dim, self.hidden_dim], 0.0, 1.0))
        self.Uog = tf.Variable(tf.random_normal([self.hidden_dim, self.hidden_dim], 0.0, 1.0))
        self.bog = tf.Variable(tf.random_normal([self.hidden_dim, ], 0.0, 1.0))

        self.Wc = tf.Variable(tf.random_normal([self.emb_dim, self.hidden_dim], 0.0, 1.0))
        self.Uc = tf.Variable(tf.random_normal([self.hidden_dim, self.hidden_dim], 0.0, 1.0))
        self.bc = tf.Variable(tf.random_normal([self.hidden_dim, ], 0.0, 1.0))
        params.extend([
    ...
    def create_output_unit(self, params):
        self.Wo = tf.Variable(tf.random_normal([self.hidden_dim, self.num_vocabulary], 0.0, 1.0))
        self.bo = tf.Variable(tf.random_normal([self.num_vocabulary], 0.0, 1.0))
